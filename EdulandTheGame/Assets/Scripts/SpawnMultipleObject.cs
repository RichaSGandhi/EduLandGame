...
public class SpawnMultipleObject : MonoBehaviour
{
		...
		if (objectsSpawned > 10) {
			int hc = PlayerPrefs.GetInt (Application.loadedLevelName + "HighScore");
			if (PlayerMove.score > hc)
				PlayerPrefs.SetInt (Application.loadedLevelName + "HighScore", PlayerMove.score);
			Debug.Log (Application.loadedLevelName + "High Score");
